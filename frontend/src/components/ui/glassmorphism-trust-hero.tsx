      ...
      <div className="relative z-10 mx-auto max-w-7xl px-4 pt-32 pb-12 sm:px-6 md:pt-40 md:pb-20 lg:px-8">
        <div className="grid grid-cols-1 gap-12 lg:grid-cols-12 lg:gap-8 items-center">
          ...
          <div className="flex flex-wrap justify-center gap-3 sm:gap-4 mb-8 sm:mb-10 animate-fade-in-up" style={{ animationDelay: "200ms" }}>
            <div onClick={() => {
                onNavigate?.('Decks')
                const docId = localStorage.getItem('yukti_current_doc_id')
                if (docId) {
                   setTimeout(() => {
                      // Trigger generate modal event
                      window.dispatchEvent(new CustomEvent('yukti:openFlashcardsGenerate'))
                   }, 300)
                }
            ...
            <div onClick={() => {
                ...
                setTimeout(() => {
                   window.dispatchEvent(new CustomEvent('yukti:triggerGraphRegeneration'))
                }, 500)
            }} className="cursor-pointer group flex items-center gap-2 px-3 sm:px-4 py-2 rounded-full bg-[#16161f]/80 border border-[#7c5cbf]/20 hover:border-[#7c5cbf]/50 hover:bg-[#7c5cbf]/10 transition-all duration-300">
            ...
            <div onClick={() => {
                ...
                const docId = localStorage.getItem('yukti_current_doc_id')
                if (docId) {
                   const saved = localStorage.getItem(`yukti_notes_${docId}`)
                   if (!saved || JSON.parse(saved).length === 0) {
                      setTimeout(() => {
                         window.dispatchEvent(new CustomEvent('yukti:triggerNotesSummarize'))
                      }, 500)
                   }
                }
            }} className="cursor-pointer group flex items-center gap-2 px-3 sm:px-4 py-2 rounded-full bg-[#16161f]/80 border border-[#7c5cbf]/20 hover:border-[#7c5cbf]/50 hover:bg-[#7c5cbf]/10 transition-all duration-300">
            ...
            <p className="animate-fade-in delay-300 max-w-xl text-lg text-[#9896b0] leading-relaxed font-medium">
              Yukti — your AI study companion that learns how you learn. Master concepts faster, generate dynamic flashcards, and map your knowledge intuitively.
            </p>
